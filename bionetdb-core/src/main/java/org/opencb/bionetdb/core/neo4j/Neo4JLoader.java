...
//    }
    ...
    public Node loadReportedVariant(ClinicalVariant clinicalVariant) {
        Node reportedVariantNode = createNeo4JNode(NodeBuilder.newNode(0, clinicalVariant));

        // Process variant and relation it to the reported variant
        Node variantNode = loadVariant(clinicalVariant);
    ...
    public Node loadReportedEvent(ClinicalVariantEvidence clinicalVariantEvidence) {
        Node reportedEventNode = createNeo4JNode(NodeBuilder.newNode(0, clinicalVariantEvidence));
...
//                                log.info("SO term accession " + so.getAccession() + " not found.");
    ...
    public Node loadPhenotype(Phenotype phenotype) {
        ...
        Node ontologyTermNode = loadOntologyTermAnnotation(phenotype);
...
//    }
    ...
    public Node loadPanel(DiseasePanel panel) {
        ...
        // Phenotypes
//        if (CollectionUtils.isNotEmpty(panel.getDisorders())) {
//            for (OntologyTerm disorder : panel.getDisorders()) {
//                // Phenotype node and relation panel - disorder
//                //Node phenotypeNode = loadOntologyTerm(disorder);
//                Node phenotypeNode = loadDisorder(disorder);
//                panelNode.createRelationshipTo(phenotypeNode, withName(PANEL__DISORDER.name()));
//            }
//        }
...
//                addOntologyTerms(panelRegion.getPhenotypes(), panelRegionNode, withName(PANEL_STR__ONTOLOGY_TERM.name()));
    ...
    private void addOntologyTerms(List<OntologyTermAnnotation> ontologyTerms, Node node, RelationshipType relation) {
        if (CollectionUtils.isNotEmpty(ontologyTerms)) {
            for (OntologyTermAnnotation ontologyTerm : ontologyTerms) {
                Node ontologyTermNode = loadOntologyTermAnnotation(ontologyTerm);
