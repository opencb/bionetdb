...
public class RestResponse<T> {
    ...
    private List<BioNetDBResult<T>> responses;

    public RestResponse() {
        this("", -1, new ArrayList<>(), new ObjectMap(), new ArrayList<>());
    }

    public RestResponse(ObjectMap params, List<BioNetDBResult<T>> responses) {
        this("", -1, new ArrayList<>(), params, responses);
    }

    public RestResponse(String apiVersion, int time, List<Event> events, ObjectMap params, List<BioNetDBResult<T>> responses) {
     ...
     */
    public List<BioNetDBResult<T>> responses() {
        return this.responses;
    }

    /**
     * Fetch the BioNetDBResult of the m-response.
     * @param m Position of the response from the array of responses.
     * @return the BioNetDBResult of the m-response.
     */
    public BioNetDBResult<T> response(int m) {
        return this.responses.get(m);
    }

    /**
     * This method just returns the first BioNetDBResult of response, or null if response is null or empty.
     * @return the first BioNetDBResult in the response
     */
    public BioNetDBResult<T> first() {
    ...
    /**
     * This method returns the first result from the first BioNetDBResult of response, equivalent to response.get(0).getResult.get(0).
    ...
    public int allResultsSize() {
        int totalSize = 0;
        if (responses != null && responses.size() > 0) {
            for (BioNetDBResult<T> dataResult : responses) {
    ...
    /**
     * This method flats the two levels (RestResponse and BioNetDBResult) into a single list of T.
    ...
    public List<BioNetDBResult<T>> getResponses() {
        return responses;
    }

    public RestResponse<T> setResponses(List<BioNetDBResult<T>> responses) {
        this.responses = responses;
        return this;
    }
